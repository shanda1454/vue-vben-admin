  ...
  setup() {
    ...
    const toolbarLang = ref(
      ...
      (() => {
        try {
          ...
          if (savedLang) {
            console.log('从localStorage读取到已保存的工具栏语言偏好:', savedLang);
    ...
    const applyToolbarTranslation = () => {
      try {
        ...
        const toolbarTranslations = {
          ...
          'bpmn-icon-connection-multi': isEnglish ? 'Global Connect Tool' : '连接工具',
          
          // 元素类
          'bpmn-icon-start-event-none': isEnglish ? 'Create Start Event' : '创建开始事件',
          'bpmn-icon-intermediate-event-none': isEnglish ? 'Create Intermediate Event' : '创建中间事件',
          'bpmn-icon-end-event-none': isEnglish ? 'Create End Event' : '创建结束事件',
          'bpmn-icon-gateway-none': isEnglish ? 'Create Gateway' : '创建网关',
          'bpmn-icon-task': isEnglish ? 'Create Task' : '创建任务',
          'bpmn-icon-subprocess-expanded': isEnglish ? 'Create Sub Process' : '创建子流程',
          'bpmn-icon-data-object': isEnglish ? 'Create Data Object' : '创建数据对象',
          'bpmn-icon-data-store': isEnglish ? 'Create Data Store' : '创建数据存储',
          'bpmn-icon-participant': isEnglish ? 'Create Pool/Participant' : '创建参与者',
          'bpmn-icon-group': isEnglish ? 'Create Group' : '创建分组',
          'bpmn-icon-text-annotation': isEnglish ? 'Create Text Annotation' : '创建文本注释'
        };
    ...
    const startPolling = () => {
      ...
      const poll = () => {
        ...
        if (success) {
          // 翻译成功，停止轮询
          console.log(`轮询成功: 第${pollState.retryCount}次尝试找到工具栏元素并完成翻译`);
          pollState.isPolling = false;
        } else if (pollState.retryCount < pollState.maxRetries) {
          // 未成功且未达到最大尝试次数，继续轮询
          console.log(`轮询中: 第${pollState.retryCount}次尝试未找到工具栏元素，继续尝试...`);
    ...
    const toggleToolbarLang = () => {
      ...
      if (!success) {
        ...
        // 成功时显示消息通知
        message.success(toolbarLang.value === 'en' 
    ...
    watch(
      ...
      (newLocale) => {
        if (newLocale) {
          ...
          // 自动更新工具栏语言，与系统保持一致
          if ((isEnglish && toolbarLang.value !== 'en') || 
    ...
    function registerBpmnEventListeners() {
      ...
      try {
        ...
        console.error('注册BPMN事件监听器出错:', err);
      }
    }
    ...
    const debugCurrentLanguage = () => {
      ...
      try {
        ...
        // 检查DOM
        console.log('HTML lang属性:', document.documentElement.lang || '未设置');
        ...
        // 检查localStorage
        console.log('localStorage中的语言设置:', localStorage.getItem('bpmn-toolbar-lang') || '未设置');
        console.log('localStorage中的其他语言相关设置:', 
  ...
  <div class="bpmn-designer-container" ref="containerRef">
    <ACard :bordered="false" class="mb-4">
      <ASpace>
        ...
        />
        <ADivider type="vertical" />
        <AButton @click="toggleToolbarLang" :type="toolbarLang === 'en' ? 'primary' : 'default'">
